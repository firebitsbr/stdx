...
use libsodium_sys::{
    ...
    crypto_generichash_KEYBYTES_MIN, crypto_generichash_final, crypto_generichash_init, crypto_generichash_state,
...
impl State {
    ...
    pub fn update(&mut self, data: &[u8]) -> Result<(), Error> {
        let rc = unsafe { crypto_generichash_update(&mut self.state, data.as_ptr(), data.len() as c_ulonglong) };
        if rc == 0 {
            Ok(())
        } else {
            Err(Error::Unknown)
    ...
    pub fn finalize(mut self) -> Result<Digest, Error> {
        ...
        let rc = unsafe { crypto_generichash_final(&mut self.state, result.data.as_mut_ptr(), result.len) };
        if rc == 0 {
            Ok(result)
        } else {
            Err(Error::Unknown)
...
mod test {
    ...
    fn test_vector_2() {
        ...
        let x = [
            0x54, 0x68, 0x65, 0x20, 0x71, 0x75, 0x69, 0x63, 0x6b, 0x20, 0x62, 0x72, 0x6f, 0x77, 0x6e, 0x20, 0x66, 0x6f,
            0x78, 0x20, 0x6a, 0x75, 0x6d, 0x70, 0x73, 0x20, 0x6f, 0x76, 0x65, 0x72, 0x20, 0x74, 0x68, 0x65, 0x20, 0x6c,
            0x61, 0x7a, 0x79, 0x20, 0x64, 0x6f, 0x67,
        ];
        let h_expected = [
            0x01, 0x71, 0x8c, 0xec, 0x35, 0xcd, 0x3d, 0x79, 0x6d, 0xd0, 0x00, 0x20, 0xe0, 0xbf, 0xec, 0xb4, 0x73, 0xad,
            0x23, 0x45, 0x7d, 0x06, 0x3b, 0x75, 0xef, 0xf2, 0x9c, 0x0f, 0xfa, 0x2e, 0x58, 0xa9,
        ];
        let mut hasher = State::new(32, None).unwrap();
        hasher.update(&x).unwrap();
        let h = hasher.finalize().unwrap();
