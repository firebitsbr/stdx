...
pub fn memcmp(x: &[u8], y: &[u8]) -> bool {
    ...
    unsafe { libsodium_sys::sodium_memcmp(x.as_ptr() as *const _, y.as_ptr() as *const _, x.len()) == 0 }
}
...
/// is used only once.
/// If the caller does not do that the cryptographic primitives in stdx_crypto
