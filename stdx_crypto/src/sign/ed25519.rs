...
pub fn keypair_from_seed(seed: &Seed) -> (PublicKey, SecretKey) {
    ...
    let mut sk = SecretKey([0u8; SECRETKEYBYTES]);
    unsafe {
        libsodium_sys::crypto_sign_ed25519_seed_keypair(pk.0.as_mut_ptr(), sk.0.as_mut_ptr(), seed.0.as_ptr());
    }
    (pk, sk)
}
...
impl State {
    ...
    pub fn update(&mut self, m: &[u8]) {
        unsafe {
            libsodium_sys::crypto_sign_ed25519ph_update(&mut self.0, m.as_ptr(), m.len() as c_ulonglong);
        }
    }
    ...
    pub fn finalize(mut self, &SecretKey(ref sk): &SecretKey) -> Signature {
        ...
        let mut siglen: c_ulonglong = 0;
        unsafe {
            libsodium_sys::crypto_sign_ed25519ph_final_create(&mut self.0, sig.as_mut_ptr(), &mut siglen, sk.as_ptr());
        }
        assert_eq!(siglen, SIGNATUREBYTES as c_ulonglong);
        Signature(sig)
    }
    ...
    pub fn verify(&mut self, &Signature(ref sig): &Signature, &PublicKey(ref pk): &PublicKey) -> bool {
        let mut sig = *sig;
        let ret =
            unsafe { libsodium_sys::crypto_sign_ed25519ph_final_verify(&mut self.0, sig.as_mut_ptr(), pk.as_ptr()) };
        ret == 0
    }
}
